...
function handleRender(req, res) {
    axios.get('http://localhost:3001/books')
        .then(function (response) {
            ...
            // STEP 3 IMPLEMENT REACT-ROUTER ON THE SERVER TO INTERCEPT CLIENT
            // REQUEST AND DEFINE WHAT TO DO WITH THEM
            const Routes = {
                routes: routes,
                location: req.url
            };
            match(Routes, function (err, redirect, props) {
                if(err){
                    res.status(500).send("Error fullfillng the request");
                }else if(redirect){
